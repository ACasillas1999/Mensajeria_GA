...
/**
 * API para obtener datos de reportes (solo datos, no archivos)
...
export const GET: APIRoute = async ({ locals, url }) => {
    try {
        ...
        return new Response(
            JSON.stringify({ ok: true, data, headers }),
            { headers: { "Content-Type": "application/json" } }
        );
    } catch (e: any) {
        console.error("Error getting report data:", e);
        return new Response(
            JSON.stringify({ ok: false, error: e?.message || "Error al obtener datos del reporte" }),
            { status: 500 }
        );
    }
};
