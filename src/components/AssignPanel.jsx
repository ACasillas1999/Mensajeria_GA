...
export default function AssignPanel() {
  ...
  async function load() {
    ...
    try {
      const [c, a, s] = await Promise.all([
        fetch(`/api/admin/conversations?estado=${fltEstado}&asignado=${fltAsignado}&search=${encodeURIComponent(search)}`).then(r=>r.json()),
  ...
  useEffect(() => { load(); /* eslint-disable-next-line */ }, [fltEstado, fltAsignado, fltSuc, search]);
    ...
    <div className="grid grid-cols-12 gap-4">
      ...
      <section className="col-span-12 lg:col-span-7 space-y-3">
        <div className="sticky top-14 z-10 bg-slate-950/90 backdrop-blur px-3 py-2 rounded-lg border border-slate-800 flex flex-wrap gap-2 items-center">
          ...
          <button onClick={load} className="ml-auto px-3 py-2 rounded bg-slate-800 hover:bg-slate-700 text-sm">Refrescar</button>
        </div>
              ...
              {asignadas.map(c => (
                <div key={c.id} className="p-3 hover:bg-slate-900/40">
                  ...
                  <div className="mt-2 flex gap-2">
                    ...
                    <button onClick={()=>assign(c.id, null)} className="px-2 py-1 rounded bg-slate-800 hover:bg-slate-700 text-sm">
                      Quitar
                    </button>
                    <a href={`/mensajes?conversation_id=${c.id}`} className="px-2 py-1 rounded bg-slate-800 hover:bg-slate-700 text-sm">
                      Ver chat
