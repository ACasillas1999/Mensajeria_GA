...
export default function ChannelSettingsModal({
  ...
  useEffect(() => {
    if (isOpen && channel) {
      ...
      setWritePermission(channel.write_permission || "all");
      setInvitePermission(channel.invite_permission || "admins");
      setError("");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `${BASE}/api/internal/channels/${channel.id}`.replace(/\/\//g, "/"),
        {
          method: "PUT",
      ...
      if (data.ok) {
        ...
        setError(data.error || "Error al actualizar canal");
      }
    } catch (err) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (data.ok) {
        ...
        setError(data.error || "Error al eliminar canal");
        setShowDeleteConfirm(false);
      }
    } catch (err) {
          ...
          <div className="px-6 py-4 space-y-4">
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-white focus:ring-2 focus:ring-emerald-500 focus:border-transparent outline-none transition-all"
              >
                <option value="all">Todos los miembros</option>
                <option value="admins">Solo admins y owner</option>
