...
export default function AgentAudit() {
  ...
  useEffect(() => {
    if (viewMode !== 'agent-detail' || !selectedAgent) return;
    let cancelled = false;
    ...
    async function loadAudit() {
      try {
        ...
        if (j.ok) {
          ...
          setStatusAudit({
            loading: false,
            error: j.error || 'Error cargando reporte',
            statuses: [],
            summary: null,
            cycles: [],
          });
        ...
        if (cancelled) return;
        console.error('Error loading status audit:', err);
        setStatusAudit({
          loading: false,
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="bg-white dark:bg-slate-950 border border-slate-300 dark:border-slate-800 rounded-xl overflow-hidden shadow-sm">
          ...
          {!statusAudit.loading && statusAudit.error && (
            <div className="px-4 pb-4 text-sm text-rose-600 dark:text-rose-400">{statusAudit.error}</div>
          )}
          {!statusAudit.loading && !statusAudit.error && statusColumns.length === 0 && (
            <div className="px-4 pb-4 text-sm text-slate-500 dark:text-slate-400">Sin datos de estatus.</div>
          )}
          {!statusAudit.loading && !statusAudit.error && statusColumns.length > 0 && (
            <div className="space-y-4 pb-4">
              <div>
                <div className="px-4 pb-2 text-sm font-medium text-slate-800 dark:text-slate-200">Resumen global</div>
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead className="bg-slate-50 border-y border-slate-300 dark:bg-slate-900/60 dark:border-slate-800">
                      <tr>
                        ...
                        <th className="text-center px-4 py-2 text-xs font-medium text-slate-600 dark:text-slate-400">Ciclos</th>
                        {statusColumns.map((st) => (
                          <th key={st.id} className="text-center px-3 py-2 text-xs font-medium text-slate-600 dark:text-slate-400">
                            {st.name}
                          </th>
                        ))}
                      </tr>
                    ...
                    <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                      <tr>
                        <td className="px-4 py-3 text-sm text-slate-900 dark:text-slate-100">{agent?.nombre || '-'}</td>
                        ...
                        {statusColumns.map((st) => (
                          <td key={st.id} className="px-3 py-3 text-center text-sm text-slate-600 dark:text-slate-300">
              ...
              <div>
                <div className="px-4 pb-2 text-sm font-medium text-slate-800 dark:text-slate-200">Por conversacion y ciclo</div>
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead className="bg-slate-50 border-y border-slate-300 dark:bg-slate-900/60 dark:border-slate-800">
                      <tr>
                        <th className="text-left px-4 py-2 text-xs font-medium text-slate-600 dark:text-slate-400">Conversacion</th>
                        <th className="text-center px-4 py-2 text-xs font-medium text-slate-600 dark:text-slate-400">Ciclo</th>
                        {statusColumns.map((st) => (
                          <th key={st.id} className="text-center px-3 py-2 text-xs font-medium text-slate-600 dark:text-slate-400">
                            {st.name}
                          </th>
                    ...
                    <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                      {cycleRows.length === 0 && (
                        <tr>
                          <td colSpan={2 + statusColumns.length} className="px-4 py-3 text-sm text-slate-500 dark:text-slate-400 text-center">
                      ...
                      {cycleRows.map((cycle) => (
                        <tr key={cycle.cycle_id}>
                          <td className="px-4 py-3">
                            <div className="text-sm font-medium text-slate-900 dark:text-slate-100">#{cycle.conversation_id}</div>
                          ...
                          <td className="px-4 py-3 text-center text-sm text-slate-600 dark:text-slate-300">
                            <div className="flex items-center justify-center gap-2">
                              ...
                              {cycle.is_active && (
                                <span className="text-[10px] px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700 border border-emerald-300 dark:bg-emerald-500/20 dark:text-emerald-300 dark:border-emerald-700">
                                  Activo
                                </span>
                              )}
                            </div>
